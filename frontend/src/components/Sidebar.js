...
const Sidebar = () => {
  ...
  return (
    <div
      className={`flex flex-col bg-[#283046] text-white h-screen transition-width duration-300 ease-in-out ${isExpanded ? "w-52" : "w-20"}`}
    ...
    >
      ...
      <div className="flex items-center my-4 px-4">
        ...
        <span className={`ml-4 text-xl font-semibold ${isExpanded ? "block" : "hidden"}`}>
          TimeTrack
        </span>
      </div>
      ...
      <div className="mt-auto px-4 py-2">
        <button
          className={`flex items-center text-center justify-center bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded ${isExpanded ? "w-full" : "w-12 justify-center"}`}
        ...
        {isDayIn && (
          <div className={`flex items-center text-center bg-white hover:bg-white text-gray-400 font-bold py-2 px-4 rounded mt-2 ${isExpanded ? "block" : "hidden"}`}>
